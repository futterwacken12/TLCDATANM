...
$(document).on('change', '[data-type="cascade-dropdown-list"]', function (e) {
    ...
    var secondFilter = $(this).attr('data-second-filter');
    // Valor seleccionado
    ...
    if (selectedId != "") {
        ...
        if (secondFilter != "")
            postData[secondFilter] = $('#' + secondFilter).val();
...
function KeepAlive() {
    ...
    http_request.open('GET', keepAliveUrl);
...
$(function () {
    ...
    //appHub.client.actualizarNroNotificaciones = function (nroNotificaciones) {

    //  $("#nro-notificaciones").text(nroNotificaciones);
    //};

    //appHub.client.registrarNotificacion = function (fecharegistro, workflow, etapa, avance, notificacion_id) {

    //    var template =  '<div class="notification">' + 
    //                        '<div class="notification-title text-danger">#ETAPA#</div>' +
    //                        '<div class="notification-description"><strong>#WORKFLOW#.</strong></div>' +
    //                        '<div class="notification-ago"><strong>Avance: #AVANCE# %</strong></div>' +
    //                        '<div class="notification-ago"><strong>#FECHAREGISTRO#</strong></div>' +
    //                        '<a href="/Workflow/Pendientes?prospectoSeleccionado=#NOTIFICACION_ID#"><div class="notification-icon fa fa-hdd-o bg-danger"> </div></a>' +
    //                    '</div>'

    //    template = template.replace("#FECHAREGISTRO#", fecharegistro)
    //                       .replace("#WORKFLOW#", workflow)
    //                       .replace("#ETAPA#", etapa)
    //                       .replace("#AVANCE#", avance)
    //                       .replace("#NOTIFICACION_ID#", notificacion_id);

    //    $template = $(template);
    //    $('#lst-notificaciones').append($template);
    //    $template.show('1000')
